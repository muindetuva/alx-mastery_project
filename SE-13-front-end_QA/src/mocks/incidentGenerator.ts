...
const scenarios = [
  ...
  ["Authentication latency", "Sign-in requests are exceeding the response-time objective."],
  ["Queue depth warning", "Background jobs are arriving faster than workers can process them."],
];

function weightedSeverity(randomValue: number): Severity {
  return severityWeights.find(({ threshold }) => randomValue < threshold)
    ?.severity ?? "low";
}

export function generateIncident(randomValue = Math.random()): Incident {
  const scenario = scenarios[
    Math.floor(randomValue * scenarios.length) % scenarios.length
  ];
...
export function generateIncidents(count: number): Incident[] {
  return Array.from(
    { length: count },
    (_, index) => generateIncident((index + 0.5) / count),
  );
}

export function generateIncidentStream(
